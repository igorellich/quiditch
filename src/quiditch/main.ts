...
import { Box3Helper, BoxGeometry, Mesh, MeshBasicMaterial, Scene } from "three";
...
const sceneManager = new ThreeSceneManager({height:window.innerHeight, width:window.innerWidth},canvas);



const bodyFactory = new RapierBodyFactory(world);
const meshFactory = new ThreeMeshFactory(scene,0);
