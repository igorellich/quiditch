import { ActiveCollisionTypes, ActiveEvents, ColliderDesc, JointData, RigidBody, RigidBodyDesc, World } from "@dimforge/rapier2d-compat";

import { createCircleBuffer32Array } from "src/tools";
...
import { RapierBasedBody } from "src/engine/rapier/RapierBasedBody";
...
export class RapierBodyFactory implements IQuiditchFactory<IBody>{
    ...
    _createBallRigidBody(): RigidBody {
        ...
        const rigidBody = this._world.createRigidBody(characterDesc);
        let characterColliderDesc = ColliderDesc.ball(0.6).setMass(0.01).setRestitution(1).setFriction(0)
